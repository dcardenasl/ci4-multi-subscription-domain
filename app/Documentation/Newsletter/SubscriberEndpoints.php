<?php

declare(strict_types=1);

namespace App\Documentation\Newsletter;

use OpenApi\Attributes as OA;

/**
 * OpenAPI definitions for Subscriber endpoints.
 *
 * @OA\Tag(name="Newsletter", description="Newsletter management")
 */
class SubscriberEndpoints
{
    #[OA\Get(
        path: '/api/v1/newsletter/subscribers',
        tags: ['Newsletter'],
        summary: 'List Subscribers',
        responses: [
            new OA\Response(
                response: 200,
                description: 'List retrieved successfully',
                content: new OA\JsonContent(
                    properties: [
                        new OA\Property(property: 'status', type: 'string', example: 'success'),
                        new OA\Property(
                            property: 'data',
                            type: 'array',
                            items: new OA\Items(ref: '#/components/schemas/SubscriberResponse')
                        ),
                    ],
                    type: 'object'
                )
            ),
        ]
    )]
    public function index(): void
    {
    }

    #[OA\Post(
        path: '/api/v1/newsletter/subscribers',
        tags: ['Newsletter'],
        summary: 'Create new Subscriber',
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(ref: '#/components/schemas/SubscriberCreateRequest')
        ),
        responses: [
            new OA\Response(response: 201, description: 'Created successfully'),
            new OA\Response(response: 422, description: 'Validation error')
        ]
    )]
    public function store(): void
    {
    }

    #[OA\Post(
        path: '/api/v1/newsletter/subscribers/import',
        tags: ['Newsletter'],
        summary: 'Bulk import Subscribers into a project',
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(ref: '#/components/schemas/SubscriberImportRequest')
        ),
        responses: [
            new OA\Response(
                response: 200,
                description: 'Import processed (per-row results in body)',
                content: new OA\JsonContent(ref: '#/components/schemas/SubscriberImportResult')
            ),
            new OA\Response(response: 404, description: 'Project not found'),
            new OA\Response(response: 422, description: 'Validation error')
        ]
    )]
    public function import(): void
    {
    }

    #[OA\Get(
        path: '/api/v1/newsletter/subscribers/{id}',
        tags: ['Newsletter'],
        summary: 'Get Subscriber by ID',
        parameters: [
            new OA\Parameter(name: 'id', in: 'path', required: true, schema: new OA\Schema(type: 'integer'))
        ],
        responses: [
            new OA\Response(
                response: 200,
                description: 'Found',
                content: new OA\JsonContent(ref: '#/components/schemas/SubscriberResponse')
            ),
            new OA\Response(response: 404, description: 'Not found')
        ]
    )]
    public function show(): void
    {
    }

    #[OA\Put(
        path: '/api/v1/newsletter/subscribers/{id}',
        tags: ['Newsletter'],
        summary: 'Update existing Subscriber',
        parameters: [
            new OA\Parameter(name: 'id', in: 'path', required: true, schema: new OA\Schema(type: 'integer'))
        ],
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(ref: '#/components/schemas/SubscriberUpdateRequest')
        ),
        responses: [
            new OA\Response(
                response: 200,
                description: 'Updated successfully',
                content: new OA\JsonContent(ref: '#/components/schemas/SubscriberResponse')
            ),
            new OA\Response(response: 404, description: 'Not found'),
            new OA\Response(response: 422, description: 'Validation error')
        ]
    )]
    public function update(): void
    {
    }

    #[OA\Delete(
        path: '/api/v1/newsletter/subscribers/{id}',
        tags: ['Newsletter'],
        summary: 'Delete Subscriber by ID',
        parameters: [
            new OA\Parameter(name: 'id', in: 'path', required: true, schema: new OA\Schema(type: 'integer'))
        ],
        responses: [
            new OA\Response(response: 204, description: 'Deleted successfully'),
            new OA\Response(response: 404, description: 'Not found')
        ]
    )]
    public function delete(): void
    {
    }
}
