# Release procedure — ci4-multi-subscription-domain

This document describes how to publish a new release of `ci4-multi-subscription-domain`. The repo is versioned **only by git tags** (`composer.json` does not carry a `version` field — it is `type: project`). A tag push on `main` triggers `.github/workflows/release.yml`, which extracts the matching `## [VERSION]` block from `CHANGELOG.md` and creates the corresponding GitHub Release.
...
5. **Verify the release page.** Open `https://github.com/dcardenasl/ci4-multi-subscription-domain/releases/tag/vX.Y.Z` and confirm the notes match the `[X.Y.Z]` block of `CHANGELOG.md`. If the workflow extracted an empty body, the most likely cause is a heading mismatch (stray trailing spaces or wrong version-string casing).
